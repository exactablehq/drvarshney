...
interface WebThreadsProps {
  ...
  className?: string;
  /** Internal render resolution as a fraction of the container's CSS size (0–1).
   *  The canvas is always stretched to 100% via CSS, so a soft glow effect like this
   *  loses no perceptible detail while rendering far fewer fragment-shader pixels. */
  renderScale?: number;
...
const WebThreads = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      antialias: false,
      // Render at a fraction of the device's pixel density — the canvas is stretched
      // to 100% via CSS, and a blurred/glowing effect like this shows no visible
      // difference while cutting fragment-shader work by 3-4x on retina displays.
    ...
    const loop = (t: number) => {
      ...
      // Throttle to targetFPS — a soft background weave doesn't need 60fps,
      // and skipping fragment-shader draws is the single biggest GPU-time saver here.
    ...
    // renderScale/targetFPS are read once at setup time by design, same as the rest of
    // this mount-only effect — changing them at runtime would require tearing down and
    // recreating the WebGL context anyway.
